...
const menu: Record<
  Role,
  { label: string; to: string; icon: typeof FiHome }[]
> = {
  CUSTOMER: [
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 overflow-x-hidden">
      <div className="flex min-h-screen">
        ...
        {/* Sidebar */}
        <aside
          className={`fixed inset-y-0 left-0 z-40 flex w-72 -translate-x-full flex-col bg-[#0b1a33] px-4 py-5 text-slate-300 transition-transform lg:translate-x-0 ${open ? "translate-x-0" : ""
            }`}
        >
          ...
          <div className="mx-3 mt-8 rounded-xl border border-white/10 bg-white/5 px-3 py-3">
            <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#ffb703]">
              {role.replace("_", " ")} portal
            </p>
            <p className="mt-1 truncate text-sm font-semibold text-white">
              {user?.firstName || "Disha user"}
            </p>
          </div>
          ...
          <div className="mt-auto border-t border-white/10 pt-4">
            <button
              onClick={logout}
              className="mt-1 flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium hover:bg-white/10 hover:text-white"
            >
        ...
        {/* Main Content */}
        <section className="min-w-0 flex-1 overflow-x-hidden lg:pl-72">
          <header className="flex h-[72px] items-center justify-between border-b border-slate-200 bg-white px-5 py-4 lg:px-8">
            ...
            <div className="hidden lg:block">
              <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">
                India road freight platform
              </p>
            </div>

            <div className="flex items-center gap-3">
              <span className="hidden text-sm text-slate-500 sm:block">
                Need help? <b className="text-slate-800">+91 6306239625</b>
              </span>
              <div className="grid h-9 w-9 place-items-center rounded-full bg-[#e8f1ff] text-sm font-bold text-[#155eef]">
                {user?.firstName?.[0] || "D"}
              </div>
            </div>
          </header>

          <div className="p-5 pb-12 lg:p-8 lg:pb-12">
            <Outlet />
          </div>
        </section>
      </div>
